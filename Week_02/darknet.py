...
class Darknet(nn.Module):
    ...
    def _create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:  
                layers.append(CNNBlock(in_channels, int(x[1]), kernel_size=int(x[0]), stride=int(x[2]), padding=int(x[3])))
                in_channels = int(x[1])
                ...
                for i in range(x[-1]):
                    for conv in x[:-1]:
                        layers.append(CNNBlock(in_channels, int(conv[1]), kernel_size=int(conv[0]), stride=int(conv[2]), padding=int(conv[3])))
