...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        mask = None
        #TODO
        attn_output = self.residual1(x, lambda x: self.self_attn(x, x, x, mask))
        attn_output = self.norm1(attn_output)
        
        # 2. Feed-forward with residual connection, dropout, and layer normalization
        ff_output = self.residual2(attn_output, self.ff)
